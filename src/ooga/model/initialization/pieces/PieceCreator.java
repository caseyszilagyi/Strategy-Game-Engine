...
public class PieceCreator extends Creator {
  ...
  private Map<String, List<Node>> restrictionNode;
  private Map<String, List<Node>> moveRestrictions;
  ...
  private List<Restriction> addRestrictions(Node moveDetails, GamePiece correspondingPiece) {
    ...
    restrictionNode = makeSubNodeMap(moveDetails);
    if (restrictionNode.containsKey(RESTRICTION_TAG)) {
      moveRestrictions = makeSubNodeMap(restrictionNode.get(RESTRICTION_TAG).get(0));
